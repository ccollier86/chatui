...
export function Sidebar({ className, onToggle, isMinimized }: SidebarProps) {
	...
	const chatsToRender = hasHydrated ? recentChats : [];
	const showEmptyState = hasHydrated ? recentChats.length === 0 : true;
	...
	if (isMinimized) {
		return (
			<div
				className={cn('flex h-full w-16 flex-col border-r bg-background', className)}
				role="navigation"
				aria-label="Sidebar"
			>
