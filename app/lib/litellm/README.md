...
The client provides comprehensive RAG capabilities including embeddings, vector store search, and reranking.

### Embeddings

Generate vector embeddings from text for semantic search:
...
- `dimensions` (optional): Output dimensions (OpenAI text-embedding-3 only)
- `encodingFormat` (optional): 'float' or 'base64'

### Vector Store Search

Search vector databases for relevant content:
...
const response = await userClient.search({
  ...
  maxResults: 5,
  scoreThreshold: 0.7,
  fileIds: ['file-123', 'file-456'] // Optional: filter by files
})
...
**Options:**
- `vectorStoreId` (required): ID of the vector store
- `query` (required): Search query string or array
- `maxResults` (optional): Maximum number of results
- `scoreThreshold` (optional): Minimum relevance score
- `fileIds` (optional): Filter results by file IDs
- `rewriteQuery` (optional): Enable query optimization

### Reranking

Rerank search results by relevance to improve RAG quality:
...
- `documents` (required): Array of documents to rank
- `model` (optional): Reranking model (default: 'rerank-english-v3.0')
- `topN` (optional): Number of top results to return
- `returnDocuments` (optional): Include document text in results

### Complete RAG Pipeline

Combine all three for optimal retrieval:

```typescript
// 1. Embed the query
const embedResponse = await userClient.embed({
  input: 'What is reinforcement learning?'
})

// 2. Search vector store
const searchResponse = await userClient.search({
  vectorStoreId: 'vs_knowledge_base',
  query: 'What is reinforcement learning?',
  maxResults: 20
})

// 3. Rerank results for better relevance
const rerankResponse = await userClient.rerank({
  query: 'What is reinforcement learning?',
  documents: searchResponse.results.map(r => r.content),
  topN: 5
})

// 4. Use top results in chat
const topResults = rerankResponse.results
  .sort((a, b) => b.relevanceScore - a.relevanceScore)
  .slice(0, 3)
  .map(r => searchResponse.results[r.index].content)
  .join('\n\n')

const chatResponse = await userClient.chat({
  messages: [
    {
      role: 'system',
...
console.log(chatResponse.content) // High-quality, context-aware answer
```
